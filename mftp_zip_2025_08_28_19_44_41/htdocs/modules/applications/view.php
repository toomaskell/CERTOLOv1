...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'upload_file') {
        ...
        try {
            ...
            // Validate file
            $fileErrors = Security::validateFileUpload($_FILES['file']);
            if (!empty($fileErrors)) {
            ...
            $filename = Security::generateUniqueFilename($_FILES['file']['name']);
            $uploadPath = UPLOAD_APPLICATIONS . 'criteria/' . $filename;
            
            // Create directory if not exists
            $uploadDir = UPLOAD_APPLICATIONS . 'criteria/';
            if (!is_dir($uploadDir)) {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadPath)) {
                ...
                $stmt = $db->query(
                    ...
                    [
                        ...
                        'file_path' => 'applications/criteria/' . $filename,
                        'file_size' => $_FILES['file']['size'],
                        'file_type' => strtolower(pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION))
                    ]
                );
                
                // Get the inserted file with user info
...
$statusTextColors = [
    ...
    'under_review' => 'text-dark',      // Dark text on yellow background
    'approved' => 'text-white',
    'rejected' => 'text-white',         // White text on red background
    'issued' => 'text-white'
];
...
<!-- Custom styles for white text on criteria and discussion badges -->
<style>
.badge.bg-success,
.badge.bg-warning, 
.badge.bg-danger,
    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            <div class="col">
                ...
                <h2 class="page-title">
                    Application Details
                    <span class="badge <?php echo $statusBadges[$application['status']] ?? 'bg-secondary'; ?> <?php echo $statusTextColors[$application['status']] ?? 'text-white'; ?> ms-2">
                            ...
                            <div class="mb-4 pb-4 border-bottom">
                                <div class="d-flex align-items-start">
                                    ...
                                    <div class="flex-fill">
                                        ...
                                        <div class="mt-3">
                                            <h5>Files</h5>
                                            <div class="files-container" id="files-<?php echo $criterion['id']; ?>">
                                                <?php if (empty($files)): ?>
                                            ...
                                            <!-- Upload File Form (only if application is under review) -->
                                            ...
                                            <div class="mt-2">
                                                ...
                                                <small class="form-hint">Allowed: PDF, Word, JPG, PNG (Max: <?php echo MAX_FILE_SIZE / 1024 / 1024; ?>MB)</small>
                                            </div>
                                            <?php endif; ?>
                                        </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body text-center">
                        <div class="mb-3">
                            <span class="badge <?php echo $statusBadges[$application['status']] ?? 'bg-secondary'; ?> <?php echo $statusTextColors[$application['status']] ?? 'text-white'; ?>" style="font-size: 1.2rem; padding: 0.5rem 1rem;">
                                <?php echo ucwords(str_replace('_', ' ', $application['status'])); ?>
                            </span>
                        </div>
                        ...
                        <?php if ($application['status'] === 'approved' && $userRole === ROLE_CERTIFIER): ?>
                            <a href="/certificates/generate/<?php echo $applicationId; ?>" class="btn btn-success w-100">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle file upload
    document.querySelectorAll('.btn-upload-file').forEach(function(button) {
        button.addEventListener('click', function() {
            const criteriaId = this.getAttribute('data-criteria-id');
            const input = document.querySelector('.file-input[data-criteria-id="' + criteriaId + '"]');
            ...
            if (file.size > <?php echo MAX_FILE_SIZE; ?>) {
                alert('File size exceeds limit of <?php echo MAX_FILE_SIZE / 1024 / 1024; ?>MB');
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    // Add file to list
                    const filesContainer = document.getElementById('files-' + criteriaId);
                    
                    // Remove "no files" message if exists
                    const noFiles = filesContainer.querySelector('.text-muted');
                    if (noFiles && noFiles.textContent === 'No files uploaded yet.') {
                        ...
                        filesContainer.appendChild(listGroup);
                    }
                    
                    // Get or create list group
                    let listGroup = filesContainer.querySelector('.list-group');
                    if (!listGroup) {
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col">
                                ...
                                <div class="text-muted small">
                                    ...
                                    <span class="badge bg-secondary">${data.file.role.charAt(0).toUpperCase() + data.file.role.slice(1)}</span>
                                    on ${data.file.uploaded_at}
                                    (${(data.file.file_size / 1024).toFixed(1)} KB)
                                </div>
                            </div>
                            <div class="col-auto">
                                <a href="/uploads/${data.file.file_path}" 
                                   target="_blank" 
                                   class="btn btn-sm btn-primary">
                                    <i class="ti ti-download"></i>
                                </a>
            ...
            .finally(() => {
                // Re-enable button
                button.disabled = false;
                button.innerHTML = '<i class="ti ti-upload"></i> Upload';
            });
        });
    });
});
